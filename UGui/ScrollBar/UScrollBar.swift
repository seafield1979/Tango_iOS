...
public class UScrollBar {
    ...
    // バーの描画サイズ
    public func getBarWidth() -> CGFloat {
//        return bgWidth - 10.0
        return bgWidth
    }
...
//            bgColor = UScrollBar.SHOW_BAR_COLOR
    ...
    public func draw() {
        ...
        if (showType == ScrollBarShowType.ShowAllways) {
            _barLength = bgLength - UDpi.toPixel(15);
            _barPos = 7;
        }
        if (type == ScrollBarType.Horizontal) {
            barRect.x = CGFloat(_barPos)
            barRect.y = UDpi.toPixel(0)
            barRect.width = CGFloat(_barLength)
            barRect.height = getBarWidth()
        } else {
            barRect.x = UDpi.toPixel(0)
            barRect.y = CGFloat(_barPos)
            barRect.width = getBarWidth()
