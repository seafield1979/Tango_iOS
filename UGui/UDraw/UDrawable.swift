...
//        print("  UDrawable:deinit")
    ...
    public func setPos(_ x : CGFloat, _ y : CGFloat, convSKPos : Bool) {
        setPos(x, y, true, convSKPos : convSKPos)
    }
    public func setPos(_ x:CGFloat, _ y:CGFloat, _ update : Bool, convSKPos : Bool)
    {
        pos.x = x
        pos.y = y
        if update {
        ...
        if movingFrame >= movingFrameMax {
            ...
            if isMovingPos {
                setPos(srcPos.x + ((dstPos.x - srcPos.x) * ratio),
                       srcPos.y + ((dstPos.y - srcPos.y) * ratio), convSKPos: true)
            }
            if isMovingSize {
